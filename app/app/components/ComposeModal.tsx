...
import {
  ...
  AlignLeft, AlignCenter, AlignRight, AlignJustify, List, ListOrdered, ChevronDown, Folder, Mic, Minus, Check
} from "lucide-react";
...
import { getMailbox, getUserProfile, sendMessage as send_message_api, uploadAttachment, ComposeDraftData } from '@/lib/compose_mail_api';
...
export function ComposeModal({ isOpen, onClose, action = "new", initialData }: { isOpen: boolean; onClose: () => void; action?: ComposeAction; initialData?: any }) {
  ...
  const fetchIdentities = async () => {
    ...
    try {
      // Fetch both mailbox and user profile in parallel
      const [mailboxResponse, profileResponse] = await Promise.all([
        getMailbox(),
        getUserProfile()
      ]);

      if (mailboxResponse.success && mailboxResponse.data.mailbox) {
        const mailbox = mailboxResponse.data.mailbox;
        const displayName = profileResponse.success && profileResponse.data.profile 
          ? profileResponse.data.profile.displayName 
          : profileResponse.success && profileResponse.data.user
          ? profileResponse.data.user.displayName || profileResponse.data.user.username
        ...
        setFromAccount({ name: displayName, email: mailbox.email });
      } else {
        throw new Error('Failed to fetch mailbox information');
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Only set signature if fromAccount is available
    if (fromAccount) {
      ...
      // For new mail or drafts, set signature only
      if (action === 'new' || (action === 'draft' && !initialData?.body)) {
        editor.commands.setContent(`<p></p>${signature}`);
      } else if (action === 'draft' && initialData?.body) {
        editor.commands.setContent(initialData.body);
      }
    }
    
    editor.commands.focus('start');
  }, [isOpen, editor, action, fromAccount, initialData]);

  const saveDraft = async () => {
    if (!isOpen || !fromAccount) return;
    
    try {
      const draftData: ComposeDraftData = {
        subject: subjectInput,
        to: toInput.split(',').map(e => e.trim()).filter(Boolean).map(email => ({ email })),
        cc: ccInput.split(',').map(e => e.trim()).filter(Boolean).map(email => ({ email })),
        bcc: bccInput.split(',').map(e => e.trim()).filter(Boolean).map(email => ({ email })),
      ...
      if (draftId) {
        // Update existing draft - would need updateDraft API
        // For now, we'll just log
        console.log('[Draft] Update draft (not fully implemented):', draftId);
      } else {
        // Create new draft - would need createDraft API
        console.log('[Draft] Create draft (not fully implemented)');
      }
  ...
  }, [toInput, ccInput, bccInput, subjectInput, attachments, editor]);
  ...
  }, [toInput, ccInput, bccInput, subjectInput, attachments, editor?.getHTML(), isOpen, debouncedSaveDraft]);
  ...
  const sendMessage = async (scheduledTime?: string) => {
    ...
    try {
      ...
      const messageData: ComposeDraftData = {
        subject: subjectInput,
        ...
        textBody: editor?.getText(),
        attachmentIds: attachments.map(a => a.attachmentId).filter(Boolean) as string[],
        replyToMessageId: initialData?.messageId,
      };

      const response = await send_message_api(messageData, undefined);
  ...
  const discardDraft = async () => {
    // Would need deleteDraft API call
    console.log('[Discard] Draft discarded:', draftId);
    onClose();
  };

  const handleFileUpload = async (file: File) => {
    try {
      const response = await uploadAttachment(file, undefined);
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="flex items-center gap-4 text-[13px] text-gray-500 dark:text-gray-400 shrink-0 pointer-events-auto">
              ...
              <button onClick={onClose} className="cursor-pointer hover:text-red-500 transition-colors p-1">
